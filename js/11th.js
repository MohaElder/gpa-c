...
function Run()
 {
   ...
   if (GPAFinal <= 3 )
     ...
     {rank = "Error!";}
   document.getElementById("ResultArea").value = ("Your GPA is "+GPAFinal.toFixed(3)+","+rank);
}
function getGpa(subject){
  ...
  if(subject=="Chi")
    return 3*getL(subLev,subScore);
  if(subject=="Eng")
    return 5.5*getL(subLev,subScore);
  if(subject=="M2")
    return 4*getNL(subLev,subScore);
  if(subject=="M3")
    return 4*getNL(subLev,subScore);
  if(subject=="M4")
    return 4*getNL(subLev,subScore);
